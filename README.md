...
Runtime wrappers (`run-opencode.sh`, `start-terminal-emacs.sh`, `start-gui-emacs.sh`, and `enter-shell.sh`) source a shared runtime env loader that only reads `/secrets/*.env`. During `dev-up.sh`, secret files copied from `secrets-paths.txt` are renamed with a numeric prefix plus `.env` suffix so they are sourced deterministically by sort order. Non-`.env` files present directly under `/secrets` are ignored and logged as warnings.
